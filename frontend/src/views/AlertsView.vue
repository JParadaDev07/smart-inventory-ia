<template>
  <div class="space-y-6">
    ...
    <div
      v-else-if="isError"
      class="rounded-lg border border-destructive/30 bg-destructive/5 p-6 text-destructive text-sm"
    >
    ...
    <template v-else-if="alerts">
      <Card>
        <template #header>
          <span
            ...
            data-tour="alerts-low-stock-title"
          >
            {{ t("alerts.lowStock") }}
          </span>
          <span class="text-muted-foreground text-xs"
            >({{ alerts?.low_stock?.length ?? 0 }})</span
          >
        </template>
          ...
          <table class="w-full text-sm">
            <thead>
              <tr class="border-b bg-muted/30">
                <th class="h-10 px-3 text-left font-medium sm:px-4">
                  {{ t("alerts.product") }}
                </th>
                <th class="h-10 px-3 text-right font-medium sm:px-4">
                  {{ t("alerts.stock") }}
                </th>
                <th class="h-10 px-3 text-right font-medium sm:px-4">
                  {{ t("alerts.min") }}
            ...
            <tbody>
              ...
              >
                ...
                <td
                  class="px-3 py-3 text-right text-destructive font-medium sm:px-4"
                >
                  {{ p.current_stock }}
                </td>
        ...
        >
          <p
            class="text-muted-foreground text-sm order-2 sm:order-1 text-center sm:text-left"
          >
            {{
              t("alerts.pageOf", {
                current: lowStockPage,
                last: lowStockPagination.lastPage,
              })
            }}
            <span class="hidden sm:inline">
              ({{ lowStockPagination.total }} {{ t("alerts.total") }})</span
            >
          </p>
          <div class="flex gap-2 justify-center sm:order-2">
            ...
            <Button
              ...
              @click="
                lowStockPage = Math.min(
                  lowStockPagination.lastPage,
                  lowStockPage + 1,
                )
              "
            >
      ...
      <Card>
        <template #header>
          <span class="text-muted-foreground text-sm font-medium">{{
            t("alerts.recommendedPurchase")
          }}</span>
          <span class="text-muted-foreground text-xs"
            >({{ alerts?.recommended_purchase?.length ?? 0 }})</span
          >
        </template>
        <p class="text-muted-foreground mb-3 text-sm">
          {{ t("alerts.recommendedPurchaseDesc") }}
        </p>
        <div
          v-if="!alerts?.recommended_purchase?.length"
          class="py-4 text-center text-sm text-muted-foreground"
        >
          ...
          <table class="w-full text-sm">
            ...
            <tbody>
              ...
              >
                ...
                <td class="px-3 py-3 text-right font-medium sm:px-4">
                ...
                <td class="px-3 py-2 sm:px-4">
                  <Button
                    variant="ghost"
                    size="sm"
                    @click="goToIntelligence(row.product.id)"
                  >
                    {{ t("alerts.viewProduct") }}
                  </Button>
                </td>
        ...
        >
          ...
          >
            {{
              t("alerts.pageOf", {
                current: recommendedPage,
                last: recommendedPagination.lastPage,
              })
            }}
            <span class="hidden sm:inline">
              ({{ recommendedPagination.total }} {{ t("alerts.total") }})</span
            >
          </p>
          <div class="flex gap-2 justify-center sm:order-2">
            <Button
            ...
            <Button
              ...
              @click="
                recommendedPage = Math.min(
                  recommendedPagination.lastPage,
                  recommendedPage + 1,
                )
              "
            >
      ...
      <Card>
        <template #header>
          ...
          <span class="text-muted-foreground text-xs"
            >({{ alerts?.expiring_soon?.length ?? 0 }})</span
          >
        </template>
        <p class="text-muted-foreground mb-3 text-sm">
          {{ t("alerts.expiringSoonDesc") }}
        </p>
        <div
          v-if="!alerts?.expiring_soon?.length"
          class="py-4 text-center text-sm text-muted-foreground"
        >
          ...
          <table class="w-full text-sm">
            ...
            <tbody>
              ...
              >
                ...
                <td class="px-3 py-3 sm:px-4 text-muted-foreground">
                  {{ row.branch?.name ?? "—" }}
                </td>
                <td class="px-3 py-3 text-right sm:px-4">
        ...
        >
          ...
          >
            {{
              t("alerts.pageOf", {
                current: expiringSoonPage,
                last: expiringSoonPagination.lastPage,
              })
            }}
            <span class="hidden sm:inline">
              ({{ expiringSoonPagination.total }} {{ t("alerts.total") }})</span
            >
          </p>
          <div class="flex gap-2 justify-center sm:order-2">
            <Button
            ...
            <Button
              ...
              @click="
                expiringSoonPage = Math.min(
                  expiringSoonPagination.lastPage,
                  expiringSoonPage + 1,
                )
              "
            >
...
const alerts = computed(() => {
  ...
  return {
    ...
    recommended_purchase: Array.isArray(raw.recommended_purchase)
      ? raw.recommended_purchase
      : [],
